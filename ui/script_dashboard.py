...
class ScriptDashboard(ctk.CTkFrame):
    ...
    def _build_left(self):
        ...
        self._stitch_btn = ctk.CTkButton(
            lf,
            text="🌟  Stitch Arc (Call 3)",
            height=40,
            font=ctk.CTkFont(weight="bold"),
            fg_color="#8E44AD",
            hover_color="#5B2C6F",
        ...
        self._stitch_btn.pack(fill="x", padx=10, pady=2)
    ...
    def _update_pre_cost(self, *args):
        ...
        if episodes and has_script:
            self._stitch_btn.configure(state="normal")
        else:
            self._stitch_btn.configure(state="disabled")
    ...
    def _start_generation(self):
        ...
        if not series or series.startswith("("):
            ...
            return
        if not episodes:
            messagebox.showerror("No Episodes", "Check at least one episode checkbox.")
            return
        ...
        threading.Thread(
            target=self._run_generation, args=(series, episodes, m1, m2), daemon=True
        ).start()

    def _run_generation(self, series: str, episodes: list, model1_raw: str, model2_raw: str):
        """
        Call 1 — 2 episodes processed in parallel.
        Within each episode images are sent sequentially in chunks of 10.
        All stats (panels, scenes, cost, chunk progress) are printed to the log.
        """
        ...
        if hasattr(self, "_fetched_models"):
            for m in self._fetched_models:
                if m["id"] == model1_raw:
                    p = m.get("pricing", {})
                    in_c1 = float(p.get("prompt", 0))
                    out_c1 = float(p.get("completion", 0))

        print(f"{'═'*50}")
        print(f"[CALL 1] Starting — {total} episodes | 2 parallel | 10-img chunks")
        print(f"{'═'*50}")
        ...
        def _process_one(ep_index, ep):
            ...
            sess_path = os.path.join(ep_dir, "session.json")
            exp_dir   = os.path.join(ep_dir, "export")
            os.makedirs(exp_dir, exist_ok=True)
            intel_path = os.path.join(exp_dir, "story_intel.json")

            if os.path.exists(intel_path):
                print(f"[SKIP] {ep} — story_intel.json already exists")
                return ep, intel_path, "skipped", 0, 0

            print(f"\n{'─'*42}")
            print(f"[CALL 1] ▶ {ep} ({ep_index+1}/{total})")
            print(f"{'─'*42}")
            ...
            for attempt in range(MAX_ATTEMPTS):
                if self._cancel.is_set():
                    return ep, None, "cancelled", 0, 0
                try:
                    panels = script_tool.preprocess_session(sess_path)
                    if not panels:
                        print(f"[ERROR] {ep}: No panels found in session.json")
                        return ep, None, "failed", 0, 0

                    encoded = script_tool.encode_images(panels)
                    n_panels = len(encoded)
                    self._total_panels += n_panels
                    print(f"[{ep}] 📷 {n_panels} panels encoded | "
                          f"Total so far: {self._total_panels}")
                    ...
                    for p in sess:
                        if p.get("ocr_text"):
                            txt = (" ".join(p["ocr_text"])
                                   if isinstance(p["ocr_text"], list)
                                   else str(p["ocr_text"]))
                            ocr_lines.append(
                                f"Scene {p.get('scene_id', p.get('index', '?'))}: {txt}"
                            )
                    full_ocr_str = "\n".join(ocr_lines)
                    ...
                    def _prog_cb(msg, frac=0.0):
                        ...
                        self._q.put({
                            "type": "progress_base",
                            "val": (self._done_count + frac) / total,
                        })

                    def _usage_c1(in_tok, out_tok, text):
                        nonlocal ep_cost
                        chunk_cost = (in_tok * in_c1) + (out_tok * out_c1)
                        ep_cost += chunk_cost
                        self._total_cost += chunk_cost

                    story_intel = pm.execute_call_1(
                        encoded, source_url, full_ocr_str,
                        progress_callback=_prog_cb,
                        usage_callback=_usage_c1,
                    )

                    if self._cancel.is_set():
                        return ep, None, "cancelled", 0, 0

                    ep_scenes = len(story_intel.get("scenes", []))
                    with open(intel_path, "w", encoding="utf-8") as f:
                        json.dump(story_intel, f, indent=2, ensure_ascii=False)

                    print(f"\n[{ep}] ✅ DONE — {ep_scenes} scenes | "
                          f"{n_panels} panels | cost=${ep_cost:.5f}")
                    print(f"[STATS] total_panels={self._total_panels}")
                    print(f"[STATS] cost={self._total_cost:.5f}")
                    return ep, intel_path, "success", ep_scenes, n_panels

                except Exception as e:
                    if str(e) == "Cancelled by user":
                        return ep, None, "cancelled", 0, 0
                    print(f"\n[ERROR] {ep} attempt {attempt+1}/{MAX_ATTEMPTS}: {e}")
                    import traceback
                    traceback.print_exc(file=sys.stdout)
                    if attempt < MAX_ATTEMPTS - 1:
                        print(f"  ↻ Retrying {ep} in 5s…")
                        import time
                        time.sleep(5)
                    else:
        ...
        try:
            with ThreadPoolExecutor(max_workers=2) as executor:
                futures = {
                    executor.submit(_process_one, i, ep): ep
                ...
                for future in as_completed(futures):
                    ...
                    print(f"{icon} [{status.upper()}] {ep} | scenes={scenes} | "
                          f"panels={panels} | done={self._done_count}/{total} | "
                          f"cost=${self._total_cost:.5f}")
                    print(f"{'═'*50}")
                    self._q.put({
                        "type": "ep_status",
                        "msg": f"{icon} {ep} [{status}] ({self._done_count}/{total})",
                    })
                    self._q.put({
                        "type": "progress_base",
                        "val": self._done_count / total,
                    })
                    if intel_path:
                        last_intel_path = intel_path

            print(f"\n{'═'*50}")
            print(f"[CALL 1] ALL DONE — {self._done_count}/{total} episodes | "
                  f"total_panels={self._total_panels} | total_cost=${self._total_cost:.5f}")
            print(f"{'═'*50}")
            self._q.put({"type": "progress_base", "val": 1.0})
            self._q.put({"type": "done", "intel_path": last_intel_path})
    ...
    def _start_scripts(self):
        ...
        threading.Thread(
            target=self._run_scripts, args=(series, episodes, provider2), daemon=True
        ).start()

    def _run_scripts(self, series: str, episodes: list, provider: str):
        try:
            ...
            for i, ep in enumerate(episodes):
                ...
                def _script_is_valid(path):
                    """Returns True only if script.txt exists AND contains plain narration (not JSON)."""
                ...
                if not _script_is_valid(sp):
                    if os.path.exists(ip):
                        print(f"\n{'═'*42}")
                        print(f"[CALL 2] Episode {i+1}/{len(episodes)}: {ep}")
                        print(f"{'═'*42}")
    ...
    def _run_merge(self, series: str, provider: str):
        self._run_mega_merge(series, provider)
    ...
    def _run_mega_merge(self, series: str, provider: str):
        try:
            os.environ["OPENROUTER_MODEL_STITCH"] = provider
            set_key(self._env_path, "OPENROUTER_MODEL_STITCH", provider)

            episodes = self._get_episodes(series)
            
            if not episodes:
                raise ValueError("No episodes found. Run 'Generate Scripts (Call 2)' first.")

            final_out_path = os.path.join(_PROJECTS_DIR, series, "script_merged", "merged_script.txt")
            
            print(f"\n[STITCH] Total {len(episodes)} episodes will be stitched...")
            self._q.put({"type": "progress_base", "val": 0.1})
            
            pm = script_tool.ProviderManager()
            from pathlib import Path
            out_path_obj = Path(final_out_path)
            
            merged = script_tool.stitch_and_build(series, episodes, out_path_obj, pm)

            if not os.path.exists(final_out_path):
                raise RuntimeError("Stitch failed: merged_script.txt not found.")

            print(f"  ✅ Stitched script → {final_out_path}")
    ...
    def _set_processing(self, state: bool):
        ...
        if state:
            ...
            for btn in [self._gen_btn, self._script_btn, self._stitch_btn, self._vo_btn]:
